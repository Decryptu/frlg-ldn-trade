...
Companion pages: [JoySpot discovery](joyspot.md) for why the Wireless
Communication path is not reachable, [the trade-host design](frlg_link_trade_host.md),
[composing gifts](frlg_gift_composer.md) for authoring an event from validated stages, and the
two events with pages of their own, [the stamp rally](frlg_gift_stamp_rally.md) and
[the Porygon TM gift](frlg_gift_porygon.md).
...
the tested surface: [JoySpot discovery](joyspot.md).
...
| `bin/frlg_mg_host.py` | thin Mystery Gift CLI and run-config construction |
...
[the legendary-beast gift](frlg_gift_beast.md).
