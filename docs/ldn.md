...
application data - before anything is known about the game. `tools/ldn_scan.py` does exactly that.
...
- [The Pia layer](pia.md) - packet header formats by version, and the two families of session-key
  derivation.
- [JoySpot discovery](joyspot_discovery_findings.md) - what a real Switch advertises, read off the
  air.

## Per-game work lives elsewhere

- [FireRed and LeafGreen](protocol.md) - a GBA ROM inside an emulator, so there is a second link
