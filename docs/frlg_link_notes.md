...
Findings for both directions: the joiner (`bin/frlg_trade_join.py --live`, the Switch hosts and we are the
RFU child) and the host (`bin/frlg_trade_host.py`, `bin/frlg_mg_host.py`). Everything here was measured
...
(`scratchpad/echo_gaps.py`) are on [the native code page](frlg_rom_buffer_script.md).
...
Measured with the receive client (`bin/frlg_mg_client.py`, run mc1) against a real Mystery Gift host,
...
PROVEN ON HARDWARE, cc1-cc4. `frlg_trade_host.py --colosseum` hosts Pokemon Center 2F -> third NPC
...
See [What Mystery Gift can do](frlg_gift_untried.md).
