...
This uses the native-code path, so read [Native code on the console](frlg_rom_buffer_script.md) for how the
...
the secret ID, which appears nowhere in the game and travels in no link message.
    ...
    (you)  sudo -E ./.venv/bin/python -u bin/frlg_mg_host.py \
...
so reaches IWRAM, including `gRngValue` - see [the random number generator](frlg_rom_rng.md), which uses that
