...
|---|---|---|
| `CLI_SAVE_RAM_SCRIPT` (17) | stores a **field script**; it runs on the next NPC interaction | every gift we ship - [Composing gifts](frlg_gift_composer.md) |
| `CLI_RUN_MEVENT_SCRIPT` (15) | runs a **Mystery Event bytecode script**, a second VM with its own 17-opcode table [`data/mystery_event_script_cmd_table.s`] | [The Mystery Event VM](frlg_rom_mystery_event.md) |
| `CLI_RUN_BUFFER_SCRIPT` (21) | `func = (void *)gDecompressionBuffer; func(&param, gSaveBlock2Ptr, gSaveBlock1Ptr)` - up to 1024 bytes of our own ARM executed with both save-block pointers [`mystery_gift_client.c:274`] | [Native code on the console](frlg_rom_buffer_script.md) |

## Done

- **Wonder Cards + field scripts** - items, Pokemon, eggs, sprites, legendary battles, stamp
  rallies. [Composing gifts](frlg_gift_composer.md).
- **The visiting trainer** (`--gift visiting-trainer`, vt01) - a 188-byte
  ...
  [Console protocol notes](frlg_link_notes.md).
- **Wonder News on the Friend path** (`--news`, wn01) - the whole second column of the console's
  menu, and the only gift path where the console answers us. [Wonder News](frlg_gift_wonder_news.md).
...
- **Native code** (`--buffer-script`, bs01 onward) - the console's memory read and written, its ROM
  ...
  `CreateMon` landing in the player's party. [Native code on the console](frlg_rom_buffer_script.md).
- **The questionnaire as a password gate** (`--questionnaire`, mev04 refused and mev06 delivered).
  [The Mystery Event VM](frlg_rom_mystery_event.md).
...
- **Mystery Gift → Wonder Cards → Wireless Communication.** Blocked at the serial-number gate: the
  ...
  [JoySpot discovery](joyspot.md). This also puts the 4x "big" Wonder News reward
...
`--game-data-log PATH` on `bin/frlg_mg_host.py` appends one record per session to a jsonl ledger
(`pokeldn/frlg/gift/game_data_log.py`), and the host prints what moved since the last session of that same
console. `tools/frlg/game_data_read.py PATH` reads the ledger back; `--session N` re-parses one session's
...
[The French Easy Chat vocabulary](frlg_rom_easy_chat_french.md).
...
that u16 has always been zero in our block. `frlg_trade_host.py --card-flag-id N` sets it.
...
**The colosseum is now built: `frlg_trade_host.py --colosseum`.** Pokemon Center 2F -> third NPC ->
...
[Console protocol notes](frlg_link_notes.md#the-cable-club-colosseum). Two things decide
