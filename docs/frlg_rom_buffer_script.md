...
Mystery Event opcode or link message. [Reading the save](frlg_rom_save.md) is the how-to;
`tools/frlg/dump_read.py` decodes what comes back.
...
[the French Easy Chat vocabulary](frlg_rom_easy_chat_french.md) is what they read.
...
during a link reaches an encounter. [The RNG page](frlg_rom_rng.md) is what went through that wall instead,
...
[on the RNG page](frlg_rom_rng.md).
...
regions merged, so a block straddling two runs still walks - and `tools/frlg/script_read.py
...
constant is sound in a way naming a save address never is. [The RNG page](frlg_rom_rng.md) is what wanted it.
...
`tools/frlg/rom_functions.py` makes it one, and it is bs84's method with the table taken out of it. A
...
would catch - the same idea as `tools/frlg/script_read.py`'s, because a run should be spent where the
