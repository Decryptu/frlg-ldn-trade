...
RFC1042_SNAP_PREFIX = b"\xAA\xAA\x03\x00\x00\x00"
...
class DataFrame:
    ...
    def accept_decrypted_ccmp(self) -> bool:
        ...
        if not self.protected or not self.payload.startswith(RFC1042_SNAP_PREFIX):
            return False
        if len(self.payload) < 16:  # Eight-byte SNAP header and CCMP MIC.
            raise ValueError("Decrypted protected frame is too short")
...
class AccessPoint(Interface):
    ...
    async def _process_disassociation(
        ...
        management_type = "deauthentication" \
            if isinstance(frame, DeauthenticationFrame) else "disassociation"
        await self._events.put(DisassociationEvent(
            frame.source, frame.reason, management_type
        ))
