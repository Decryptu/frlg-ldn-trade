...
class APNetwork:
    ...
    async def _process_events(self) -> None:
        while True:
            ...
            if isinstance(event, wlan.CustomFrameEvent):
                ...
                await self._process_disassociation(
                    event.address, event.reason, event.management_type
                )
    ...
    async def _process_disassociation(
        self, address: MACAddress, reason: int | None = None,
        management_type: str | None = None
    ) -> None:
        ...
        await self._events.put(LeaveEvent(
            index, participant, reason, management_type
        ))
        ...
        while True:
            ...
            if isinstance(frame, wlan.DataFrame):
                try:
                    ...
                    logger.debug(f"Ignoring invalid data frame: {e}")
    ...
    async def _process_data_frame(self, frame: wlan.DataFrame) -> None:
        ...
        if frame.protected:
            ...
            accepted_plaintext = (
                self._param.accept_decrypted_ccmp and
                frame.accept_decrypted_ccmp()
            )
            if not accepted_plaintext:
