...
def _distribution():
    return compile_definition(event.WORDS_XP_GIFT)
...
def test_definition_compiles_to_the_expected_card_and_cli_entry():
    ...
    assert int.from_bytes(card[0:2], "little") == event.WORDS_XP_GIFT_FLAG_ID
...
def test_revisit_requires_the_kanto_pokedex_then_gives_celebi():
    ...
    assert complete.sprites == [
        (wonder_card.OBJ_EVENT_GFX_CELEBI, 0, 10, 18, 3, wonder_card.DIR_SOUTH),
    ]
    assert complete.moves == [
        (7, 0, wonder_card.MOVE_HEAL_BELL),
        (7, 1, wonder_card.MOVE_SAFEGUARD),
    ]
